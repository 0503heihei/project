<?php
session_start();
?>
<!DOCTYPE html>
<html>

	<head>
		<meta charset="UTF-8">
		<title>葡萄创新科技产品一站式选购平台，儿童益智玩具首选</title>
		<link rel="shortcut icon" href="img/favicon.ico" type="image/x-icon">
		<link rel="stylesheet" type="text/css" href="css/reset.css" />
		<link rel="stylesheet" type="text/css" href="css/mall.css" />
		<link rel="stylesheet" type="text/css" href="css/footer.css"/>
	</head>

	<body>
		<!--导航栏-->
		<div class="header">
			<div class="header_inner">
				<a href="index.php" class="logo"></a>
				<ul class="header_nav">
					<li>
						<a href="index.php">首页</a>
					</li>
					<li>
						<a href="brand.php">品牌动态</a>
					</li>
					<li>
						<a href="theatre.php">葡星剧场</a>
					</li>
					<li>
						<a href="pt_home.php">葡萄之家</a>
					</li>
					<li>
						<a href="app.php">应用下载</a>
					</li>
				</ul>
				<div class="login_info">
					<a href="signin.php" class="login">登录</a>
					<!-- 动态登录 -->



					<a href="signup.html" class="shop_mall">注册</a>
				</div>
			</div>
		</div>
		<!--主体-->
		<section>
			<div class="top_banner"></div>
			<div class="prduct_list">
				<div class="prduct_title">
					科技玩具
					<span>
						Technology toys
					</span>
				</div>
				<div class="prduct_con">
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/5a430d756059d58b6b4cc0dfc3d9f96e1b3a8fe4.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>奇妙发现</span>
							<i>快乐不止于屏幕，虚拟+现实儿童科技益智玩具(本产品需搭配iPad使用)</i>
							<a href="###">
								599.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/1d6902f98ed789030f6b0e4f1cde1b7f07150ee1 (1).jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>涂涂世界</span>
							<i>用灵感点亮世界，虚拟＋现实儿童科技益智玩具（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								299.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/a2f2babaea44932cc1f45705f8c741ad54619187.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>星球魔方</span>
							<i>智能识别教学还原魔方（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								99.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/1d6902f98ed789030f6b0e4f1cde1b7f07150ee1.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>哈妮</span>
							<i>潜入想象力的海洋，虚拟＋现实儿童科技益智玩具（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								299.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/95bc3f7b88ed2ad66c8411fbde187c2355df4932 (1).jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>涂涂世界</span>
							<i>用灵感点亮世界，虚拟＋现实儿童科技益智玩具（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								299.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/fe9de7037f4e2d6d9475a1811068e73382f15582.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>麦思思</span>
							<i>智能识别教学还原魔方（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								299.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/027cbb0d85bb983e2f4cc589be1c99dc8227d619.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>Hello编程</span>
							<i>智能识别教学还原魔方（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								299.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/4cc0fc9bafbbc00a411b63143d5da8da011e43c4.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>奇妙电路</span>
							<i>智能识别教学还原魔方（本产品需搭配“探索号飞船”使用）</i>
							<a href="###">
								499.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_title">
						科技玩具
						<span>
						Technology toys
					</span>
					</div>
					<div class="prduct_title">
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/7d42690290ca85ebeda92a1bb316b09de5b5c242.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>探索号底座-平板支架（标准版）</span>
							<i>带你探索奇妙世界，虚拟＋现实儿童科技益智玩具（本产品需搭配ipad使用）</i>
							<a href="###">
								59.00
								<b>元</b>
							</a>
						</p>
					</div>
					<div class="prduct_info">
						<p class="prduct_img">
							<a href="###">
								<img src="img/shangcheng/871f3f31eb12352a2c1dfb473e996e669793f6ba.jpg" />
							</a>
						</p>
						<p class="prduct_text">
							<span>探索号底座-平板支架（豪华版）</span>
							<i>带你探索奇妙世界，虚拟＋现实儿童科技益智玩具（本产品需搭配ipad使用）</i>
							<a href="###">
								99.00
								<b>元</b>
							</a>
						</p>
					</div>
				</div>
			</div>

		</section>
		<footer>
			<div id="footer">
				<div class="bottom-nav">
					<a href="index.html" class="bottom-logo"><img src="img/bottom/logo-bottom.png" /></a>
					<ul>
						<li>
							<h3>订单中心</h3></li>
						<li>
							<a href="###">购买中心</a>
						</li>
						<li>
							<a href="###">支付指南</a>
						</li>
						<li>
							<a href="###">配送指南</a>
						</li>
					</ul>
					<ul>
						<li>
							<h3>服务支持</h3></li>
						<li>
							<a href="###">隐私政策</a>
						</li>
						<li>
							<a href="###">售后政策</a>
						</li>
						<li>
							<a href="###">产品常见问题</a>
						</li>
					</ul>
					<ul>
						<li>
							<h3>商务合作</h3></li>
						<li>
							<a href="###">商务合作</a>
						</li>
					</ul>
					<ul>
						<li>
							<h3>关于葡萄</h3></li>
						<li>
							<a href="###">关于葡萄</a>
						</li>
						<li>
							<a href="###">加入葡萄</a>
						</li>
						<li>
							<a href="###">联系我们</a>
						</li>
					</ul>
					<ul>
						<li>
							<h3>关注我们</h3></li>
						<li>
							<a href="###" id="pt_code">葡萄维度</a>
							<img id="qcode" style="position: absolute; z-index: 1; top:80px; display: none; right: 60px; width: 160px;" src="img/bottom/7eab92fb77eafce2b9cc886a6d220846ea812a32.png" />
						</li>
					</ul>
					<script type="text/javascript">
						document.getElementById('pt_code').onmouseenter = function() {
							document.getElementById('qcode').style.display = 'block';
						}
						document.getElementById('pt_code').onmouseleave = function() {
							document.getElementById('qcode').style.display = 'none';
						}
					</script>
					<div class="online">
						<p class="tel">400-651-6161</p>
						<span>周一至周五 9:00-22:00</span>
						<span>仅收市话费</span>
					</div>
				</div>
			</div>
			<div class="copyright">
				<div class="copyright-inner">
					<h2>Copyright © 2016, Shanghai Putao Technology Co., Ltd. All Rights Reserved. 葡萄科技</h2>
					<h3>葡萄科技</h3>
					<br>
					<h4>
						<a class="weihuajingying" target="_blank" href="http://sq.ccm.gov.cn/ccnt/sczr/service/business/emark/toDetail/35EE6340F6157151E0530140A8C0E25A">网络文化经营单位</a>
						<a href="http://www.miibeian.gov.cn/state/outPortal/loginPortal.action"> 沪ICP备15015904号-3</a>
					</h4>
				</div>
			</div>
		</footer>
		<div class="footer_functions">
			<a href="http://vipwebchat.sh.tq.cn/pageinfo.jsp?version=vip&amp;admiuin=9730189&amp;ltype=0&amp;iscallback=0&amp;is_message_sms=0&amp;is_send_mail=0&amp;action=acd&amp;acd=1&amp;type_code=1001" target="_blank"></a>
			<a href="#returnTop"></a>
		</div>

		<script src="js/jquery-2.1.3.min.js"></script>
		<script src="js/signin.js" type="text/javascript" charset="utf-8"></script>
	</body>

</html>